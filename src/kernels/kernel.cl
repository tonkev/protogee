...
__kernel void pre_rays(read_only image2d_t positions, constant light* vpls, const uint vplsPerPixel, const uint pwidth, const uint iss, global ray* rays){
	...
	const uint pv = vplsPerPixel * (((y % iss) * iss) + (x % iss)) + v;
...
__kernel void post_rays(constant ray* rays, constant int* occlus, const uint vplsPerPixel, const uint pwidth, const uint iss, write_only image2d_array_t vpl_masks){
	const uint x = get_global_id(0);
	const uint y = get_global_id(1);
	const uint v = get_global_id(2);
	const uint pv = vplsPerPixel * (((y % iss) * iss) + (x % iss)) + v;
