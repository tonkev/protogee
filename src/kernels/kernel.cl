...
__kernel void pre_rays(read_only image2d_t positions, constant light* vpls, const uint vplsPerPixel, const uint pwidth, const uint iss, global ray* rays){
	const uint x = get_global_id(0);
	const uint y = get_global_id(1);
	const uint v = get_global_id(2);
	const uint pv = v * (((y % iss) * iss) + (x % iss));
	const int i = ((y*pwidth + x) * vplsPerPixel) + v;
	const float3 pos = read_imagef(positions, sampler, (int2)(x, y)).xyz;
	rays[i].o = (float4) (vpls[pv].position, length(pos - vpls[pv].position) - 0.01);
	rays[i].d = (float4) (normalize(pos - vpls[pv].position), 0.f);
	rays[i].doBackfaceCulling = 0;
}
__kernel void post_rays(constant ray* rays, /*constant intersection* isects*/ constant int* occlus, const uint vplsPerPixel, const uint pwidth, const uint iss, write_only image2d_array_t vpl_masks){
	const uint x = get_global_id(0);
	const uint y = get_global_id(1);
	const uint v = get_global_id(2);
	const uint pv = v * (((y % iss) * iss) + (x % iss));
